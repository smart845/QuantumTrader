...
(function(){
  ...
  const SPREADS_BTN_ID='btnSpreads';
  ...
  async function scanSpreads(root,signal){
    ...
    const rows=[];groups.forEach(({base,quote,items})=>{if(items.length<2)return;let min={price:Infinity},max={price:-Infinity};for(const it of items){if(it.price<min.price)min=it;if(it.price>max.price)max=it;}if(!isFinite(min.price)||!isFinite(max.price)||min.price<=0)return;const pct=((max.price-min.price)/min.price)*100;if(pct<MIN_SPREAD)return;rows.push({pair:`${base}${quote}`,exMax:`${max.ex} ${max.type==='DEX'?'dex':'linear'}`,price:max.price,pct,funding:'—',deals:'—'});});rows.sort((a,b)=>b.pct-a.pct);
    if(tb){tb.innerHTML='';if(rows.length===0){if(emp)emp.style.display='block';}else{const spreadsBtn=document.getElementById(SPREADS_BTN_ID);if(spreadsBtn)spreadsBtn.classList.add('pulsate-red');for(const r of rows){const tr=document.createElement('tr');tr.dataset.pair=r.pair;tr.dataset.ex=r.exMax;const c=colorFor(r.pct);tr.innerHTML=`<td>${r.pair}</td><td>${r.exMax.replace(' linear','').replace(' dex','')}</td><td>${fmtNum(r.price,4)}</td><td style="color:${c}">${r.pct.toFixed(2)}%</td><td>${r.funding}</td><td>${r.deals}</td><td><button class="emoji-btn delete-spread-btn">🗑️</button></td>`;tr.querySelector('.delete-spread-btn').addEventListener('click',()=>{tr.remove();if(!tb.querySelector('tr')){if(emp)emp.style.display='block';const spreadsBtn=document.getElementById(SPREADS_BTN_ID);if(spreadsBtn)spreadsBtn.classList.remove('pulsate-red');}});tb.appendChild(tr);await yieldUI();}}}
  ...
  function observe(){const o=new MutationObserver(()=>{document.querySelectorAll('#modalSpreadsPro').forEach(r=>{if(r.dataset.ready)return;r.dataset.ready=1;const c=makeCtrl(r);const b=r.querySelector('#spreadsRefresh');if(b)b.addEventListener('click',()=>{c.stop();const a=new AbortController();scanSpreads(r,a.signal);});const seg=r.querySelector('#spreadsSeg');if(seg){seg.querySelectorAll('button').forEach(btn=>{btn.addEventListener('click',function(){seg.querySelectorAll('button').forEach(b=>b.classList.remove('active'));this.classList.add('active');if(this.dataset.mode==='auto'){c.start();}else{c.stop();}});});}setTimeout(()=>{c.start();},300);});});o.observe(document.documentElement,{childList:true,subtree:true});}
